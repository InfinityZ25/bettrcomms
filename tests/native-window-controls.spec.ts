...
for (const mode of ['native-frame', 'native-overlay'] as const) {
  test(`${mode} keeps one caption and follows the actual CSS palette`, async ({ page }) => {
    await page.goto('/');
    await page.evaluate(async (mode) => {
      ...
      host.__BETTER_WINDOW_CONTROLS__ = {
        platform: 'windows', mode, height: mode === 'native-overlay' ? 32 : 0,
