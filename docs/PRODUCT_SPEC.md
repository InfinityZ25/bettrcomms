...
Accepted compatibility extension: automatic microphone-only WSS fallback on the same API host, with explicit Server voice selection and an absolute direct-only override. Opus media is encrypted between clients with ephemeral keys; users can compare session codes outside signaling. Without comparison, public-key delivery trusts authenticated signaling. The relay retains no call recordings, exposes its route, bounds unsent queues, and returns to WebRTC after a fresh stable-path acknowledgment. Video and shared application audio are not included in this fallback. The initial relay and signaling share one in-memory process; separating or horizontally scaling them requires room routing first. See `VOICE_RELAY.md` for operational and trust limits.
...
Video prefers hardware encode/decode when the runtime exposes it, then degrades resolution, frame rate, and finally video availability before harming audio. Capability decisions use actual codec negotiation and runtime stats, not GPU-name guessing. Native high-fidelity modes expose 30/60/120 FPS presets, custom 15–240 FPS, and custom 1–200 Mbps with H.264 level validation; actual delivery may be lower when capture, encode, decode, or network capacity is insufficient. AV1/VP9/H.264 choices are based on mutual support and measured performance.
