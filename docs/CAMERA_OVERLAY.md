...
The window is a Rust-owned Win32 layered popup with no extra WebView, taskbar entry, audio renderer, network connection, or capture device. The existing call WebView decodes the camera tracks and composites their small previews, labels and mute/deafen/speaking state into RGBA. A bounded binary IPC transfer targets 24 FPS starting in 0.1.7. Early frames are paced rather than discarded; only one frame is in flight, so slower hardware reduces cadence without accumulating stale frames. Source tracks and recordings are untouched. This is not a wholly Rust media pipeline: moving the existing WebRTC decoders into Rust would be a separate transport project.
