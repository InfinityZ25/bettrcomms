...
Windows native capture requires the FFmpeg runtime described in NATIVE_SHARING.md. Version 0.1.1 provides an explicit in-app verified download into private app storage; a global Winget installation is optional. NVIDIA/DirectML runtimes remain separately provisioned through their documented flows; installers do not bundle the development machine's proprietary runtimes. macOS currently uses WebView-supported microphone/camera/browser capture; Windows capture, system audio and GPU denoising adapters are not macOS implementations. A successful Mac compile is not a physical-device acceptance test.
