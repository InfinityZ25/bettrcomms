...
Each viewer owns its own track, bounded send queue and paced writer task. The encoder's output thread only enqueues, so a congested viewer can no longer stall the capture pipe and degrade the stream for everyone else; that viewer drops its own frames and resumes at the next keyframe, which the per-viewer `droppedFrames` diagnostic reports. A viewer also starts on a keyframe rather than mid-GOP. Packets leave through a leaky bucket at 2.5× the selected bitrate, so a keyframe spreads over milliseconds instead of arriving as one burst. The sender's own preview connection never leaves the machine and is not paced, because there is no path capacity to pace against.
...
- Live bitrate is manually selected; native congestion-driven encoder adaptation and ICE restart are not implemented. The offer advertises only the feedback this sender acts on — NACK retransmission, PLI and FIR — and deliberately not transport-wide congestion control, because no estimator drives the encoder's bitrate yet.
- One-second IDR frames allow late joining/recovery; immediate encoder response to PLI is not implemented. PLI and FIR are received and counted but cannot force an IDR, because the encoder is a subprocess with no runtime keyframe channel. A viewer that starts or loses data therefore waits up to one second for the next scheduled IDR. In-process encoding is the prerequisite for closing this.
- Watching an unwatched share no longer waits for a keyframe: the frozen tile keeps the track attached and decoding and paints a still frame over it, so only the picture is frozen, not the decoder. The stream arrives whether or not a tile paints it, so this costs decode, not bandwidth.
- A viewer whose link cannot carry the selected bitrate cannot be helped by the encoder, which has no congestion control and one rate for everyone. Each receiver therefore measures its own two-second windows of unrecovered loss and freeze time, and after three consecutive bad windows moves itself to the compatibility route, which is lower quality but runs under the browser's own rate control. The switch is one-way, needs six seconds of sustained trouble, ignores windows carrying fewer than 100 packets, and marks the affected screen "Reduced quality". This changes the route, not the encoder; real per-viewer adaptation still needs in-process encoding.
...
NVENC uses P6 low-latency tuning, spatial AQ strength 8, temporal AQ, and full-resolution multipass. It retains zero B-frames and zero lookahead, CBR, and one-second keyframes, explicitly forced as IDR rather than relying on the GOP boundary alone.

The recovery interval moved from two seconds to one after a VMAF comparison of both intervals at 1080p, 60 and 120 FPS, and 8 and 20 Mbps on the synthetic motion/grid/text fixture:

| Frame rate | Mbps | 1 s GOP VMAF | 2 s GOP VMAF |
| ---------- | ---- | ------------ | ------------ |
| 60         | 8    | 94.445       | 94.348       |
| 60         | 20   | 98.719       | 98.688       |
| 120        | 8    | 86.887       | 86.730       |
| 120        | 20   | 95.135       | 95.274       |

Every pair falls within 0.16 VMAF, measured output rates matched the target to within 0.03 Mbps, and a repeat of 120 FPS at 20 Mbps reversed the sign (95.296 against 95.169). The interval therefore has no measurable quality cost in this range, so the shorter recovery is taken. This supersedes the earlier two-second choice, which was compared only at 1080p60 and against a five-second interval. It is not a substitute for answering a PLI.

The rate-control buffer bounds one access unit to about a tenth of a second of bits, and never to less than two frame intervals, replacing the previous half-second VBV. A measured 1080p120 capture at 20 Mbps kept the same keyframe cadence and the same mean frame size (20 943 versus 20 963 bytes) while its largest frame fell from 147 031 to 115 511 bytes, so the worst-case burst the pacer has to spread is about a fifth smaller at unchanged average quality. Encoder and SDP levels match the actual dimensions, frame rate, and bitrate. Unsupported dimension/rate combinations above H.264 Level 5.2, including 1440p240, are rejected before capture instead of advertising an invalid stream. Recording copies these same encoded access units and scales its bounded queue for high-refresh input. Stop drains pending recording frames before finalization; stopping before the first IDR produces an actionable error instead of an unusable asset.
