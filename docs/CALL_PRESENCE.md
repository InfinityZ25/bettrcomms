...
Call presence belongs to the existing signaling process's memory, not PostgreSQL. Authenticated members can read an authorized snapshot through `/api/v1/call-presence`; an open call sends validated mute/deafen state through its existing signaling socket. The web UI serially refreshes one snapshot every two seconds, never one request per room. Failed refreshes clear the displayed snapshot and mark it unavailable rather than claiming a call is empty. Disconnect, connection replacement, membership revocation, and server restart remove ephemeral state.
...
This implementation retains the current single signaling-process deployment constraint. Multi-replica operation requires shared ephemeral presence and room routing, or an explicitly designed authenticated peer presence protocol. The polling snapshot can later become a dedicated account-level subscription without changing the lobby contract. It must remain separate from the call socket so observing a room never enters the media mesh.
