import { hasNativeMediaHost, beginNativeExport, appendNativeExport, finishNativeExport, abortNativeExport } from '../desktop/nativeMedia';
...
export async function saveRecordingAsset(
  ...
  try {
    grant = await beginNativeExport(file.name, file.blob.size);
  } catch (error) {
  ...
  try {
    ...
    for (let offset = 0; offset < file.blob.size; offset += chunkSize) {
      ...
      const data = await file.blob
        ...
        .arrayBuffer();
      signal.throwIfAborted();
      await appendNativeExport(grant.exportId, offset, new Uint8Array(data));
      onProgress(Math.min(1, (offset + data.byteLength) / file.blob.size));
    }
    signal.throwIfAborted();
    const saved = await finishNativeExport(grant.exportId);
    committed = true;
    return saved;
  } finally {
    if (!committed)
      await abortNativeExport(grant.exportId).catch(() => {});
  }
}
